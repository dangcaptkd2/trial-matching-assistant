...
def create_materialized_view():
    ...
    SELECT
        ...
        (SELECT array_agg(DISTINCT f.state ORDER BY f.state) 
         FROM ctgov.facilities f 
         WHERE f.nct_id = s.nct_id AND f.state IS NOT NULL) AS states,
        (SELECT array_agg(DISTINCT f.country ORDER BY f.country) 
         FROM ctgov.facilities f 
         WHERE f.nct_id = s.nct_id AND f.country IS NOT NULL) AS countries,
        (SELECT array_agg(DISTINCT f.name ORDER BY f.name) 
         FROM ctgov.facilities f 
         WHERE f.nct_id = s.nct_id AND f.name IS NOT NULL) AS facility_names,
        (SELECT STRING_AGG(DISTINCT CONCAT_WS(', ', f.name, f.city, f.state, f.country), E'\\n') 
         FROM ctgov.facilities f 
         WHERE f.nct_id = s.nct_id) AS sites_text
    FROM
        ctgov.studies s;
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Manage materialized view for Elasticsearch indexing")
    parser.add_argument("--refresh", action="store_true", help="Refresh existing materialized view with latest data")
    parser.add_argument("--drop", action="store_true", help="Drop the materialized view")
    parser.add_argument("--info", action="store_true", help="Show information about the materialized view")
