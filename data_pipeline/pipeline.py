...
def run_download(date_str: str = None, output_dir: str = "./data/downloads", keep_zip: bool = False) -> Path | None:
    ...
    cmd = ["python", "data_pipeline/download_aact.py", "--output-dir", output_dir]
    ...
    try:
        ...
        # Find the .dmp file in version subdirectories
        output_path = Path(output_dir)

        # Look for .dmp files in all subdirectories (version folders)
        ...
        if dmp_files:
            ...
            logger.info(f"Searched in: {output_path}/*/*.dmp")
            return None
...
def run_materialized_view_refresh() -> bool:
    ...
    try:
        logger.info(f"Running command: {' '.join(cmd_refresh)}")
        # Don't capture output - let it stream to console in real-time
        subprocess.run(cmd_refresh, check=True)
...
def main():
    ...
    parser.add_argument("--config", type=str, default="data_pipeline/config.yaml", help="Path to configuration file")
    parser.add_argument("--skip-download", action="store_true", help="Skip download step (use existing dump file)")
    parser.add_argument("--skip-import", action="store_true", help="Skip PostgreSQL import step")
    parser.add_argument("--skip-indexing", action="store_true", help="Skip Elasticsearch indexing step")
    ...
    try:
        ...
        if not args.skip_download:
            dmp_file = run_download(date_str=args.date, output_dir=config["aact"]["download_dir"], keep_zip=False)
            ...
            # Find most recent .dmp file in version subdirectories
            ...
            if dmp_files:
                ...
                logger.info(f"Searched in: {download_path}/*/*.dmp")
                sys.exit(1)
        ...
        if not args.skip_cleanup:
            cleanup_old_files(config["aact"]["download_dir"], config["aact"]["retention_days"])
        else:
